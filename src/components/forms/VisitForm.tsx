...
export function VisitForm({ onClose, visit }: VisitFormProps) {
  ...
  const validate = (): boolean => {
    ...
    if (!formData.lab_result_bp) newErrors.lab_result_bp = 'Blood pressure is required';
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-sm font-medium text-foreground mb-1.5">Lab Result - BP</label>
          <input
            type="text"
            placeholder="120/80"
          ...
          />
          {errors.lab_result_bp && <p className="text-xs text-destructive mt-1">{errors.lab_result_bp}</p>}
        </div>
      </div>
