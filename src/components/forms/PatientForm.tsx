...
export function PatientForm({ onClose, patient }: PatientFormProps) {
  const { addPatient, updatePatient, generatePatientId, isPatientIdUnique, fetchPatients } = useHospital();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Refresh patients list to sync with database
      await fetchPatients();
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-center mb-6">
        <div
          className={`flex items-center justify-center w-8 h-8 rounded-full ${step >= 1 ? "gradient-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
        >
          {step > 1 ? <Check className="w-4 h-4" /> : "1"}
        </div>
        <div
          className={`w-20 h-1 mx-2 rounded ${step > 1 ? "bg-primary" : "bg-muted"}`}
        />
        <div
          className={`flex items-center justify-center w-8 h-8 rounded-full ${step >= 2 ? "gradient-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
        >
          2
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              />
              {errors.bmi && (
                <p className="text-xs text-destructive mt-1">{errors.bmi}</p>
              )}
            </div>
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      smoker_status: e.target.checked,
                    })
                  }
            ...
            <div className="flex items-center gap-3">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, alcohol_use: e.target.checked })
                  }
                  className="w-4 h-4 rounded border-border text-primary focus:ring-primary"
                />
